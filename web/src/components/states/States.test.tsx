...
describe("state components", () => {
  ...
  it("renders an empty state with a clear message", async () => {
    const { container } = render(<EmptyState message="Try changing your filters." />);
  ...
  it("renders an error state with an alert and a working, keyboard-operable retry action", async () => {
    ...
    expect(screen.getByText("Could not load data", { exact: false })).toBeInTheDocument();
