...
--   python - <<'PY' ... (see chat)
--
-- TWO THINGS TO KEEP IN STEP WITH db/b6_schema.sql, or nothing here loads:
--
--   1. IDs are TEXT, not numbers. species_id is TEXT and record_id is VARCHAR,
--      because real BRERC SPECIES_NO / unique_No values are not always numeric.
--      PostgreSQL will NOT quietly turn 100001 into '100001' on insert — it
--      raises "column is of type text but expression is of type integer". So
--      every id below is quoted.
--
--   2. Every table needs "Load" and "Load_date". They are NOT NULL with no
--      default, on purpose: the pipeline's add_load_metadata() always stamps
--      them, and a default would hide a run that forgot to. That means these
--      hand-written inserts have to set them too.
-- =============================================================================
...
-- last_updated is what /api/meta/provenance serves as the dashboard's "last
-- updated". In a real run the pipeline keeps it current via upsert_provenance()
-- (api/etl/provenance.py); this hand-written value is just a starting point.
