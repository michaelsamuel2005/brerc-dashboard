...
def _source_row(unique_no, scientific_name, common_name, easting, northing, verified="Accepted"):
    return {
        "unique_no": unique_no,
...
def _fake_generalise_locations(df, connection, easting_column, northing_column, resolution_column):
    # Stand-in for the real PostGIS round-trip: adds the columns
    # generalise_locations would add, without touching a database.
    # DELIBERATELY passes coordinates through unrounded - this test
    # file is not checking D0/D2 maths, only that these columns exist
...
def test_edited_record_updates(dictionary_df):
    with patch("etl.reconciliation.reconcile.insert_records") as mock_insert, \
        ...
        source_df = pd.DataFrame([
            _source_row(1, "Meles meles", "Badger", 400500, 300500),  # coordinates changed
        ])
        old_hash_source = add_content_hash(pd.DataFrame([
            _source_row(1, "Meles meles", "Badger", 400000, 300000),  # original values
        ]))
        ...
        assert set(updated_df["unique_no"]) == {1}
...
def test_retracted_record_disappears(dictionary_df):
    with patch("etl.reconciliation.reconcile.insert_records") as mock_insert, \
        ...
        source_df = pd.DataFrame([
            _source_row(1, "Meles meles", "Badger", 400000, 300000),
        ])
        ui_source = add_content_hash(pd.DataFrame([
            _source_row(1, "Meles meles", "Badger", 400000, 300000),
            _source_row(2, "Myotis daubentonii", "Daubenton's Bat", 401000, 301000),  # retracted
        ]))
