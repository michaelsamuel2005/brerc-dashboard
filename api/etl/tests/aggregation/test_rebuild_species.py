...
def test_rebuild_species_index_returns_early_if_empty(mock_upsert, mock_read_sql):
    ...
    rebuild_species_index(connection=mock_connection, load_mode="initial")
...
@patch("etl.aggregation.rebuild_species.pd.read_sql")
@patch("etl.aggregation.rebuild_species.upsert_species")
def test_rebuild_species_index_calculates_aggregations_correctly(
    mock_upsert, mock_read_sql
):
    ...
    rebuild_species_index(connection=mock_connection, load_mode="incremental")
...
@patch("etl.aggregation.rebuild_species.pd.read_sql")
@patch("etl.aggregation.rebuild_species.upsert_species")
def test_rebuild_species_index_passes_through_timestamp_and_mode(
    mock_upsert, mock_read_sql
):
    ...
    rebuild_species_index(
        connection=mock_connection, load_mode="incremental", load_timestamp=custom_time
    )
