...
def test_initial_load_truncates_and_copies_data():
    ...
    df = pd.DataFrame({"id": [1, 2], "species_name": ["Robin", "Blackbird"]})
    ...
    mock_cursor.copy.assert_called_once_with(
        "COPY test_table (id, species_name) FROM STDIN"
    )

    # 3. Check exact rows were written to the copy context
    expected_rows = [call((1, "Robin")), call((2, "Blackbird"))]
...
def test_incremental_load_executes_upsert():
    ...
    expected_rows = [(101, "Verified", 5), (102, "Unconfirmed", 1)]
