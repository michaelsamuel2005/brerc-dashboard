...
def test_build_admin_database_url_uses_env_var():
    ...
    with patch.dict(os.environ, {"DATABASE_URL_ADMIN": "postgresql://test_admin:pass@db:5432/test_db"}):
...
def test_get_admin_connection_opens_valid_connection():
    ...
    with patch("etl.load.reload.psycopg.connect") as mock_connect, \
         patch("etl.load.reload._build_admin_database_url", return_value="postgresql://mock_url"):
...
def test_force_full_reload_uses_existing_connection():
    ...
    with patch("builtins.open", mock_open(read_data=mock_sql_content)):
        force_full_reload(connection=mock_connection, schema_path=dummy_path)
...
def test_force_full_reload_creates_and_closes_connection():
    ...
    with patch("builtins.open", mock_open(read_data=mock_sql_content)), \
         patch("etl.load.reload.get_admin_connection", return_value=mock_connection):
