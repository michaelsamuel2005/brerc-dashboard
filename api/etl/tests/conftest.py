...
PostgreSQL ETL destination database is reachable (they check real counts and
the safe view).
...
db/b0_staging_setup.sql and pointed DESTINATION_DATABASE_URL at it.
"""
...
def database_available() -> bool:
    ...
    try:
        with get_destination_connection() as conn, conn.cursor() as cur:
...
needs_db = pytest.mark.skipif(
    ...
    reason=(
        "No ETL destination database reachable — set DESTINATION_DATABASE_URL "
        "and run db/b0_staging_setup.sql"
    ),
)
...
needs_b6_schema = pytest.mark.skipif(
    ...
    reason=("B6 schema not loaded — run db/b6_schema.sql against DESTINATION_DATABASE_URL"),
)
